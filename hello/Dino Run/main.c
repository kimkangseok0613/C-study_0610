...
#define?TREE_BOTTOM_X?45?//콘솔?창의?크기와?제목을?지정하는?함수
void?SetConsoleView()
{????
...
{????
	...
	{????????//z키가?눌렸고,?바닥이?아닐때만?점프가능.???????
		...
		{???????????
			dinoY?+=?gravity;???????
		}?????????//Y가?계속해서?증가하는걸?막기위해?바닥을?지정.????????//Y가?증가한다는?것은?공룡이?아래로?내려온다는?뜻.???????
		...
		{?????????
			isJumping?=?false;
		}?????????//나무가?왼쪽으로?가도록?하고(x의?음수방향)????????//나무의?위치가?왼쪽?끝으로가면?다시?초기?위치로?소환????????treeX?-=?2;???????
		if?(treeX?<=?0)??????
		{????????
			treeX?=?TREE_BOTTOM_X;??
